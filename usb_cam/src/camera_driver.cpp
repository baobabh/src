...
std::string AbstractV4LUSBCam::frame_id_ = "usb_cam";
...
void AbstractV4LUSBCam::grab_image(sensor_msgs::Image* msg)
{
    camera_image_t* new_image = read_frame();  // 프레임 캡처
    if (!new_image)
    {
        printf("Failed to grab image from camera.");
        return;
    }

    // 타임스탬프 설정
    msg->header.stamp = ros::Time(new_image->stamp.tv_sec, new_image->stamp.tv_nsec);
    msg->header.frame_id = frame_id_;  // frame_id_는 카메라 프레임 이름 (예: "camera")

    // 이미지 기본 정보 설정
    msg->height = new_image->height;
    msg->width = new_image->width;
    msg->encoding = new_image->encoding;
    msg->is_bigendian = false;  // 일반적으로 USB 카메라는 little-endian
    msg->step = new_image->step;

    // 이미지 데이터 복사
    size_t data_size = new_image->step * new_image->height;
    if (msg->data.size() != data_size)
        msg->data.resize(data_size);
    std::memcpy(&msg->data[0], new_image->image, data_size);
}
