...
class ControlDriving():
    def __init__(self):
        ...
        self.Vp = float(os.getenv('VP', 0.22))
        self.Kp = float(os.getenv('KP', 0.011))
        self.Kd = float(os.getenv('KD', 0.01))
        self.Ki = float(os.getenv('KI', 0.0001))
	...
	self.flag_tunnel = True
        ...
        while not rospy.is_shutdown():
            ...
            if self.moving_type == ECP_STATE.NORMAL.value:
                ...
                if self.first_time:
                    self.set_cmd_vel([0.1,0,0], [0,0,0.1])
                    rospy.sleep(1)
                    self.first_time = 0
                # rospy.loginfo('정지2')
                # self.set_cmd_vel([0, 0, 0], [0, 0, 0])
		...
		while(self.right_obstacle_count <=8 and self.flag_tunnel):
			if self.lane_pos == 0:
			    ...
			    self.set_cmd_vel([0.3, 0, 0], [0, 0, 0])
			self.right_obstacle_count = self.fn_cal_scan_count(self.scan_dis[265:275], 0.3)
    ...
    def fnIntersection(self, direction):
        left_obstacle_count = self.fn_cal_scan_count(self.scan_dis[85:95], 0.2)
        if 7 < left_obstacle_count:
        ...
        if self.lane_pos == 3:
            if direction == 1:
                self.set_cmd_vel([0.1, 0, 0], [0, 0, 0.3])
            else:
                self.set_cmd_vel([0.1, 0, 0], [0, 0, -0.3])
        else:
            self.fnNormalDriving()
    ...
    def fnConstructionMission(self):
        # 전방의 장애물 감지 (전방 좌우 20도 각도로 스캔)
        forward_obstacle_count_right = self.fn_cal_scan_count(self.scan_dis[340:359], 0.3)  # 시계방향 전방
        forward_obstacle_count_left = self.fn_cal_scan_count(self.scan_dis[0:20], 0.3)     # 반시계방향 전방

        rospy.loginfo("lane_pos: %d", self.lane_pos)
        
        # 전방에 장애물이 있을 경우
        if forward_obstacle_count_left + forward_obstacle_count_right > 5:
            rospy.loginfo('Forward Obstacle Detected')

            # 차선 위치 정보(lane_pos)를 활용한 회피 전략
            if self.lane_pos == 1 or self.lane_pos == 0:  # 왼쪽 차선에 있을 경우
                self.avoid_obstacle_left()  # 왼쪽으로 회피
            else:
                if forward_obstacle_count_left > forward_obstacle_count_right:
                    self.avoid_obstacle_left()  # 왼쪽으로 회피
                else:
                    self.avoid_obstacle_right()  # 오른쪽으로 회피

        # 장애물이 없으면 정상 주행
        else:
            self.fnNormalDriving()
        if (self.fn_cal_scan_count(self.scan_dis[0:359], 0.3)) == 0:
            self.moving_type = ECP_STATE.PARKING.value
            self.pub_sign.publish(self.moving_type)

    def avoid_obstacle_left(self):
        # 장애물과의 거리에 따라 회전 각도 동적 조정
        rospy.logerr('Turning Right to Avoid Obstacle')
        
        # 장애물이 가까울수록 더 큰 각도로 회전 (최대 90도)
        turn_angle = -1.5
        self.turn_turtlebot(turn_angle, 3)  # 회전
        rospy.logwarn("State: Move Forward")
        self.drive_forward(0.3)
        self.turn_turtlebot(-turn_angle, 3)  # 원래 방향으로 복귀

    def avoid_obstacle_right(self):
        # 장애물과의 거리에 따라 회전 각도 동적 조정
        rospy.logerr('Turning Left to Avoid Obstacle')
    ...
    def fn_cal_scan_distance(self, scan_range):
        # 주어진 스캔 범위에서 가장 가까운 장애물과의 거리 계산
        valid_distances = [d for d in scan_range if d > 0]  # 유효한 거리 데이터만 사용
        if len(valid_distances) > 0:
            return min(valid_distances)
        else:
            return float('inf')  # 유효한 데이터가 없으면 무한대 반환


    def drive_forward(self, distance):
        # 일정 거리를 직진하는 함수
        velocity = 0.15  # 이동 속도
        time_to_drive = distance / velocity  # 이동하는 데 필요한 시간 계산
        self.set_cmd_vel([velocity, 0, 0], [0, 0, 0])
        rospy.sleep(time_to_drive)  # 일정 시간만큼 직진
        
        # 직진 후 정지
        self.set_cmd_vel([0, 0, 0], [0, 0, 0])

    """def fnConstructionMission(self):
        forward_obstacle_count_right = self.fn_cal_scan_count(self.scan_dis[350:359], 0.3)
        forward_obstacle_count_left = self.fn_cal_scan_count(self.scan_dis[0: 10], 0.3)
        right_obstacle_count = self.fn_cal_scan_count(self.scan_dis[260:280], 0.1)
        left_obstacle_count = self.fn_cal_scan_count(self.scan_dis[80:110], 0.1)
        rospy.loginfo("lane_pos: %d", self.lane_pos)
        if 10 < forward_obstacle_count_left + forward_obstacle_count_right < 20:
            rospy.loginfo('Forward Obstacle Detecting')
            if (self.lane_pos == 1 or self.lane_pos == 0) or (self. first_time == 0) : #left lane
                self.turn_turtlebot(-1.4,2)
                left_obstacle_count = self.fn_cal_scan_count(self.scan_dis[90:110], 0.2)
                while left_obstacle_count:
                    left_obstacle_count = self.fn_cal_scan_count(self.scan_dis[90:110], 0.2)
                    self.set_cmd_vel([0.2, 0, 0], [0, 0, 0])
                self.turn_turtlebot(1.57,2)
                    
            elif self.first_time or (self.lane_pos == 2 and forward_obstacle_count_left<right_obstacle_count): #right lane
                self.turn_turtlebot(1.4,2)
                right_obstacle_count = self.fn_cal_scan_count(self.scan_dis[250:270], 0.2)
                print(right_obstacle_count)
                while right_obstacle_count:
                    right_obstacle_count = self.fn_cal_scan_count(self.scan_dis[250:270], 0.2)
                    self.set_cmd_vel([0.2, 0, 0], [0, 0, 0])
                self.turn_turtlebot(-1.57,2)
                self.first_time = 0
    ...
    def turn_turtlebot(self, desired_angle, time_to_rotate):
        ...
        self.set_cmd_vel([0,0,0],[0,0,angular_velocity])
    ...
    def get_lane_info(self, msg):
        ...
        error = self.desired_center - 160
        self.running_avg.add_value(self.desired_center - 160)
        average_value = self.running_avg.get_average()
        ...
        #rospy.loginfo("현재 오차: {}".format(self.desired_center - 160))
        #rospy.loginfo("현재 평균: {}\n".format(average_value))

        home_directory = os.path.expanduser('~')
        file_path = os.path.join(home_directory, 'record.txt')  #example.txt

        with open(file_path, 'a') as file:
            file.write("{}\n".format(self.desired_center - 160))
            file.write("save complete\n")
