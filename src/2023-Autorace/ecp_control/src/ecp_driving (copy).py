...
class ControlDriving():
    ...
    def fnConstructionMission(self):
        ...
        forward_obstacle_count_right = self.fn_cal_scan_count(self.scan_dis[340:359], 0.2)  # 시계방향 전방
        forward_obstacle_count_left = self.fn_cal_scan_count(self.scan_dis[0:20], 0.2)     # 반시계방향 전방

        rospy.loginfo("lane_pos: %d", self.lane_pos)
        
        # 전방에 장애물이 있을 경우
        if forward_obstacle_count_left + forward_obstacle_count_right > 15:
    ...
    def avoid_obstacle_left(self):
        ...
        forward_obstacle_count_right = self.fn_cal_scan_count(self.scan_dis[350:359], 0.2)
        forward_obstacle_count_left = self.fn_cal_scan_count(self.scan_dis[0:10], 0.2)
        while (forward_obstacle_count_right + forward_obstacle_count_left)!=0:
            self.cmd_vel([0,0,0],[0,0,0.3])
                     # 장애물이 가까울수록 더 큰 각도로 회전 (최대 90도)
            forward_obstacle_count_right = self.fn_cal_scan_count(self.scan_dis[355:359], 0.2)
            forward_obstacle_count_left = self.fn_cal_scan_count(self.scan_dis[0:5], 0.2)
    ...
    """def fnConstructionMission(self):
        forward_obstacle_count_right = self.fn_cal_scan_count(self.scan_dis[330:359], 0.3)
        forward_obstacle_count_left = self.fn_cal_scan_count(self.scan_dis[0: 30], 0.3)
        rospy.loginfo("lane_pos: %d", self.lane_pos)
